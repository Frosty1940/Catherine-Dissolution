...
if ( SERVER ) then
	...
	function PLUGIN:Infect( pl, isCritical )
		for k, v in pairs( player.GetAllByLoaded( ) ) do
			if ( v.CAT_DISSOLUTION_diseaseInfect ) then
				v.CAT_DISSOLUTION_diseaseInfect = nil
			end
		end
		...
		if ( isCritical ) then
			for k, v in pairs( ents.FindInSphere( playerPos, 200 + math.random( 10, 100 ) ) ) do
				if ( IsValid( v ) and v:IsPlayer( ) and pl != v and v:IsCharacterLoaded( ) and v:Alive( ) ) then
					...
					if ( !tr.HitWorld ) then
						v.CAT_DISSOLUTION_diseaseInfect = true
					end
				end
			end
		else
	...
	function PLUGIN:OnSpawnedInCharacter( pl )
		if ( pl.CAT_DISSOLUTION_diseaseInfect ) then
			pl.CAT_DISSOLUTION_diseaseInfect = nil
		end
		...
		local timerID = "Catherine.DISSOLUTION.plugin.disease.AutoHealCold." .. charID
	...
	function PLUGIN:PlayerThink( pl )
		...
		if ( catherine.character.GetCharVar( pl, "disease_cold_active" ) ) then
			if ( ( pl.CAT_DISSOLUTION_nextCough or 0 ) <= CurTime( ) ) then
				self:Cough( pl )
				pl.CAT_DISSOLUTION_nextCough = CurTime( ) + math.random( 5, 60 )
			end
		...
		if ( ( pl.CAT_DISSOLUTION_nextDisease or 0 ) <= CurTime( ) ) then
			if ( pl.CAT_DISSOLUTION_diseaseInfect ) then
				...
				pl.CAT_DISSOLUTION_nextDisease = CurTime( ) + math.random( 5, 30 )
			...
			pl.CAT_DISSOLUTION_nextDisease = CurTime( ) + 300
		end
	end
