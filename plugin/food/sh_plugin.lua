...
if ( SERVER ) then
	...
	function PLUGIN:PlayerThink( pl )
		if ( pl.CAT_DISSOLUTION_damageNeed ) then
			if ( pl.CAT_DISSOLUTION_damageWaitTick ) then
				if ( pl.CAT_DISSOLUTION_damageWaitTick <= CurTime( ) ) then
					pl.CAT_DISSOLUTION_damageWaitTick = nil
					pl.CAT_DISSOLUTION_damageDelay = CurTime( ) + 7
				end
			else
				pl.CAT_DISSOLUTION_damageWaitTick = CurTime( ) + 80
			end
			
			if ( pl.CAT_DISSOLUTION_damageDelay and pl.CAT_DISSOLUTION_damageDelay <= CurTime( ) ) then
				if ( pl:Alive( ) ) then
					pl:TakeDamage( math.random( 3, 7 ), pl, pl )
					pl.CAT_DISSOLUTION_damageDelay = CurTime( ) + 7
				else
					pl.CAT_DISSOLUTION_damageNeed = nil
					pl.CAT_DISSOLUTION_damageWaitTick = nil
					pl.CAT_DISSOLUTION_damageDelay = nil
				end
			end
		end
		...
		if ( ( catherine.character.GetCharVar( pl, "thirsty", 0 ) >= 95 or catherine.character.GetCharVar( pl, "hunger", 0 ) >= 95 ) and !pl.CAT_DISSOLUTION_damageNeed ) then
			pl.CAT_DISSOLUTION_damageNeed = true
		elseif ( pl.CAT_DISSOLUTION_damageNeed ) then
			pl.CAT_DISSOLUTION_damageNeed = nil
		end
		
		if ( ( pl.CAT_DISSOLUTION_nextHungerTick or 0 ) <= CurTime( ) ) then
			...
			pl.CAT_DISSOLUTION_nextHungerTick = CurTime( ) + 350 + math.random( 50, 100 )
		end
		
		if ( ( pl.CAT_DISSOLUTION_nextThirstyTick or 0 ) <= CurTime( ) ) then
			...
			pl.CAT_DISSOLUTION_nextThirstyTick = CurTime( ) + 150 + math.random( 50, 100 )
		end
	end
else
