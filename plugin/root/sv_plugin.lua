...
function PLUGIN:RootPlayer( pl, target )
	...
	netstream.Start( pl, "catherine.dissolution.plugin.root.OpenPanel", {
		target,
...
function PLUGIN:RootWork( pl, target, workID, data )
	if ( workID == CAT_ROOT_ACTION_GIVE ) then
		...
		netstream.Start( pl, "catherine.dissolution.plugin.root.RefreshPanel", {
			target,
			catherine.inventory.Get( target )
		} )
...
netstream.Hook( "catherine.dissolution.plugin.root.Work", function( pl, data )
	PLUGIN:RootWork( pl, data[ 1 ], data[ 2 ], data[ 3 ] )
end )

netstream.Hook( "catherine.dissolution.plugin.root.RootClose", function( pl )
