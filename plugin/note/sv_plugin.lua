...
function PLUGIN:DataSave( )
	...
	for k, v in pairs( ents.FindByClass( "cat_dissolution_note" ) ) do
		entData[ #entData + 1 ] = {
			pos = v:GetPos( ),
			ang = v:GetAngles( ),
...
function PLUGIN:DataLoad( )
	...
	for k, v in pairs( data.entData or { } ) do
		local ent = ents.Create( "cat_dissolution_note" )
		ent:SetPos( v.pos )
		ent:SetAngles( v.ang )
		ent:Spawn( )
	...
	catherine.entity.RegisterUseMenu( ent, {
		{
			...
			func = function( pl, ent )
				netstream.Start( pl, "catherine.dissolution.plugin.note.OpenPanel", {
					ent:EntIndex( ),
...
netstream.Hook( "catherine.dissolution.plugin.note.Write", function( pl, data )
