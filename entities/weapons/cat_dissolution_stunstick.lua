--[[
	This code has brought from Catherine HL2RP.
	https://github.com/L7D/Catherine-HL2RP
]]--
	...
	if ( SERVER and tr.Hit ) then
		...
		if ( IsValid( ent ) and ent:IsPlayer( ) ) then
			...
			if ( !ent:IsRagdolled( ) ) then
				if ( !ent.CAT_DISSOLUTION_stunCount or !ent.CAT_DISSOLUTION_ragdollRunCount ) then
					ent.CAT_DISSOLUTION_stunCount = 0
					ent.CAT_DISSOLUTION_ragdollRunCount = math.random( 4, 8 )
				end
				
				local stunCount = ent.CAT_DISSOLUTION_stunCount

				local timerID = "Catherine.DISSOLUTION.timer.Stunstick.StunCountRemover." .. ent:SteamID( )
				...
				if ( active ) then
					...
					ent.CAT_DISSOLUTION_stunCount = stunCount
				...
				if ( stunCount >= ent.CAT_DISSOLUTION_ragdollRunCount ) then
					catherine.player.RagdollWork( ent, true, 90 )
					ent.CAT_DISSOLUTION_stunCount = nil
					ent.CAT_DISSOLUTION_ragdollRunCount = nil
				...
				timer.Create( timerID, 3, stunCount, function( )
					local reStunCount = ent.CAT_DISSOLUTION_stunCount
					
					if ( reStunCount > 0 ) then
						reStunCount = reStunCount - 1

						ent.CAT_DISSOLUTION_stunCount = reStunCount
					else
						timer.Remove( timerID )
						ent.CAT_DISSOLUTION_stunCount = nil
						ent.CAT_DISSOLUTION_ragdollRunCount = nil
