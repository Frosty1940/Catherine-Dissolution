...
local ZOMBIE_SPAWN_DELAY = catherine.configs.zombieSpawnDelay

-- How many zombies there can be spawned in the map.
local ZOMBIE_MAX_COUNT = catherine.configs.zombieMaxCount
...
hook.Add( "DataSave", "Schema.DataSave", function( )
	...
	for k, v in pairs( ents.FindByClass( "cat_dissolution_static_radio" ) ) do
		data[ #data + 1 ] = {
			pos = v:GetPos( ),
			ang = v:GetAngles( ),
...
hook.Add( "DataLoad", "Schema.DataLoad", function( )
	...
	for k, v in pairs( data ) do
		local ent = ents.Create( "cat_dissolution_static_radio" )
		ent:SetPos( v.pos )
		ent:SetAngles( v.ang )
		ent:Spawn( )
		ent:SetColor( v.col )
...
function Schema:ChatPrefix( pl, classTable )
	if ( !pl:Team( ) == FACTION_HECU ) then return false end
	...
	if ( uniqueID == "ic" or uniqueID == "radio" or uniqueID == "yell" or uniqueID == "whisper" ) then
		...
		if ( uniqueID == "ic" ) then
			for k, v in pairs( ents.FindInSphere( pl:GetPos( ), 100 ) ) do
				if ( v:GetClass( ) == "cat_dissolution_static_radio" and v:GetNetVar( "active" ) and ( v:GetNetVar( "freq" ) != "XXX.X" or v:GetNetVar( "freq" ) != "" ) ) then
		...
		for k, v in pairs( ex ) do
			...
			for k1, v1 in pairs( self.vo.normalVoice ) do
				...
				if ( lowerText == v1.command:lower( ) ) then
					...
					if ( ( team == FACTION_CITIZEN or team == FACTION_GUARD ) and pl:IsFemale( ) and v1.allowFemale ) then
...
function Schema:PlayerUseDoor( pl, ent )
	...
	if ( ( pl:Team( ) == FACTION_HECU ) and !ent:HasSpawnFlags( 256 ) and !ent:HasSpawnFlags( 1024 ) ) then
		ent:Fire( "Open", "", 0 )
...
function Schema:PlayerFootstep( pl, pos, foot, soundName, vol )
	if ( !pl:IsRunning( ) or pl:Team( ) == FACTION_CITIZEN ) then return false end
	
	local team = pl:Team( )
	local sound = nil
		
	if ( team == FACTION_GUARD or team == FACTION_HECU ) then
		sound = "npc/combine_soldier/gear" .. math.random( 1, 6 ) .. ".wav"
	end
	
	if ( team == FACTION_CEDA ) then
		sound = "npc/metropolice/gear" .. math.random( 1, 6 ) .. ".wav"
	end
	pl:EmitSound( sound, 45 )
...
function Schema:GetPlayerPainSound( pl )
	local team = pl:Team( )
	if ( !team == FACTION_HECU or !team == FACTION_CEDA  ) then return end
	
	if ( team == FACTION_HECU or team == FACTION_CEDA ) then
...
function Schema:OnSpawnedInCharacter( pl )
	if ( pl:Team( ) == FACTION_HECU ) then
		pl:SetMaxHealth( 255 )
		pl:SetHealth( 255 )
		pl:SetArmor( 255 )
		
		return
	elseif ( pl:Team( ) == FACTION_CEDA ) then
		pl:SetArmor( 255 )
...
function Schema:ChatTypingChanged( pl, bool )
	...
	if ( !pl:Alive( ) or !team == FACTION_CEDA or !team == FACTION_HECU ) then return end
				...
				if ( v1:HasItem( "portable_radio" ) ) then
					...
					if ( targetItemDatas.freq == v.freq and targetItemDatas.toggle and targetItemDatas.freq and ( targetItemDatas.freq != "xxx.x" and targetItemDatas.freq != "" ) ) then
						...
						for k2, v2 in pairs( ents.FindInSphere( v1:GetPos( ), 100 ) ) do
							if ( v2:GetClass( ) == "cat_dissolution_static_radio" and v2:GetNetVar( "active" ) and v2:GetNetVar( "freq" ) != "XXX.X" and v2:GetNetVar( "freq" ) != "" ) then
			...
			if ( v:HasItem( "portable_radio" ) ) then
				...
				if ( targetItemDatas.freq == playerFreq and targetItemDatas.toggle and playerToggle and targetItemDatas.freq and ( targetItemDatas.freq != "xxx.x" and targetItemDatas.freq != "" ) ) then
					...
					for k1, v1 in pairs( ents.FindInSphere( v:GetPos( ), 100 ) ) do
						if ( v1:GetClass( ) == "cat_dissolution_static_radio" and v1:GetNetVar( "active" ) and v1:GetNetVar( "freq" ) != "XXX.X" and v1:GetNetVar( "freq" ) != "" ) then
